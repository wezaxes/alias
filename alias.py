...
# --- ГРА (DISCORD SYNC) ---
    ...
    if not data.get("explainer"):
        ...
        quotes = [
            ...
            "📐 Поки чекаємо, можеш повирішувати приклади з математики Мерзляка за 7 клас: Розв'яжіть рівняння: 4(x - 5) = 3x - 6",
            "📐 Поки чекаємо, можеш повирішувати приклади з математики Мерзляка за 7 клас: Спростіть вираз: (a - 4)(a + 2) - (a - 1)²",
            "📐 Поки чекаємо, можеш повирішувати приклади з математики Мерзляка за 7 клас: Розкладіть на множники: 25x² - 16y²",
            "📐 Поки чекаємо, можеш повирішувати приклади з математики Мерзляка за 7 клас: Знайдіть значення виразу: 2x³ - 3x² + x, якщо x = -1",
            "📐 Поки чекаємо, можеш повирішувати приклади з математики Мерзляка за 7 клас: Подайте у вигляді степеня: (x⁴)² * x³",
            "📐 Поки чекаємо, можеш повирішувати приклади з математики Мерзляка за 7 клас: Розв'яжіть систему рівнянь методом подстановки: x + y = 5 та 2x - y = 1",
            "📐 Поки чекаємо, можеш повирішувати приклади з математики Мерзляка за 7 клас: Обчисліть: (-0,2)⁴ * 5⁴",
            "📐 Поки чекаємо, можеш повирішувати приклади з математики Мерзляка за 7 клас: Спростіть одночлен: -3a²b * 4a³b⁴",
            "📐 Поки чекаємо, можеш повирішувати приклади з математики Мерзляка за 7 клас: Розв'яжіть рівняння: |x + 3| = 7",
            "📐 Поки чекаємо, можеш повирішувати приклади з математики Мерзляка за 7 клас: Розкладіть на множники способом групування: ax + ay + 3x + 3y",
            "📐 Поки чекаємо, можеш повирішувати приклади з математики Мерзляка за 7 клас: Знайдіть кути рівнобедреного трикутника, якщо кут при вершині 40°",
            "📐 Поки чекаємо, можеш повирішувати приклади з математики Мерзляка за 7 клас: Доведіть тотожність: (x - 3)(x + 3) = x² - 9",
            "📐 Поки чекаємо, можеш повирішувати приклади з математики Мерзляка за 7 клас: Розв'яжіть рівняння: (x + 2)/3 - (x - 1)/2 = 1",
            "📐 Поки чекаємо, можеш повирішувати приклади з математики Мерзляка за 7 клас: Спростіть вираз: (3m - n)² - (3m + n)²",
            "📐 Поки чекаємо, можеш повирішувати приклади з математики Мерзляка за 7 клас: Побудуйте графік функції y = 2x - 3 (яку точку перетинає вісь Oy?)",
            "📐 Поки чекаємо, можеш повирішувати приклади з математики Мерзляка за 7 клас: Знайдіть суму та різницю многочленів: 2x² - 3x + 1 та x² + 3x - 4",
            "😁 Ми теж не знаємо що таке Барбадос і як це пояснити."
        ]
        ...
        if is_host:
            if st.button("ПОЧАТИ ГРУ 🎲"):
                ...
                if len(players) >= 2:
                    ...
                    st.error("Ты нарушал обещание,чёрт тайлер что здесь происходит почему меня принимают за тебя?! Сядь,Так почему меня принимают за тебя? А ты не знаешь?нет не знаю, нет знаешь, нет,нет ... Я не знаю, вспомни,почему все вокруг путают нас? Потому что, потому что....Потому что мы одни человек? Правильно.")
        else:
            st.warning("⏳ Очікуємо, поки хост розбереться як запустити хід...")
    else:
