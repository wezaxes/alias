...
if st.session_state.game_state == "welcome":
    ...
    if st.session_state.game_mode == "discord":
        room_id = st.text_input("Код кімнати (наприклад: 777):", value=" ").upper()
...
# --- СИНХРОНІЗОВАНЕ ЛОББІ (DISCORD) ---
    ...
    st.write("Гравці:", ", ".join(data["players"]))
...
# --- БІЧНА ПАНЕЛЬ ДЛЯ ВИХОДУ ---
    with st.sidebar:
        ...
        if st.button("🔴 ВИЙТИ ДО НАЛАШТУВАНЬ"):
            # Повертаємо на екран налаштувань саме для Discord
            st.session_state.game_state = "setup"
    ...
    if not data.get("explainer"):
        ...
        quotes = [
            ...
            "🍎 Хто програє — той іде мити яблука.",
