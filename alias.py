...
# --- ГРА (DISCORD SYNC) ---
    ...
    # Визначаємо, чи є поточний користувач хостом
    is_host = (data.get("host") == my_name)

    # --- ЛОГІКА СПОВІЩЕНЬ (Toast) ---
    ...
    if not data.get("explainer"):
        ...
        # Тільки хост бачить кнопку початку
        if is_host:
            if st.button("ПОЧАТИ ГРУ 🎲"):
            ...
            st.warning("⏳ Очікуємо, поки Хост запустить раунд...")

    else:
        rem = int(data["t_end"] - time.time())
        if rem <= 0:
            st.warning("Час вийшов!")
            # Тільки хост бачить кнопку переходу до наступної пари
            if is_host:
        ...
        # Для синхронізації стану таймера та дій хоста
        time.sleep(1)
        st.rerun()
