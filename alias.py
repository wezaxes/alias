...
# ==========================================
# 1. НАЛАШТУВАННЯ СТОРІНКИ ТА СТИЛІ
# ==========================================
...
# --- ЕКРАН 4: НАЛАШТУВАННЯ (SETUP) ---
    ...
    # --- ДОДАВАННЯ СЛІВ (ОРИГІНАЛЬНА ЛОГІКА) ---
    st.divider()
...
# ==========================================
# 6. СИНХРОНІЗОВАНЕ ЛОББІ ТА ГРА (DISCORD)
# ==========================================
    ...
    if st.session_state.game_state == "sync_lobby":
        ...
        if is_host:
            ...
            if st.button("ПОЧАТИ ГРУ ДЛЯ ВСІХ 🔥"):
                ...
                ref.update(
                    {"state": "playing", "current_round": 1, "explainer": "", "listener": "", "scores": start_scores})
                st.rerun()
        else:
        ...
        with st.sidebar:
            st.write(f"👤 Ти: **{st.session_state.my_name}** {'(Адмін 👑)' if is_host else ''}")
            st.write(f"🏠 Кімната: **{st.session_state.room_id}**")
            st.divider()
            ...
            for p in current_players:
                ...
                st.caption(f"• {p}: {score} балів {'(ти)' if p == st.session_state.my_name else ''}")
            st.divider()
            if st.button("🔴 ВИЙТИ"):
                new_list = [p for p in current_players if p != st.session_state.my_name]
                ref.update({"players": new_list})
        ...
        with game_container.container():
            ...
            if not data.get("explainer"):
                ...
                if is_host:
                    if st.button("ПОЧАТИ ГРУ 🎲"):
                        ...
                        if len(players) >= 2:
                            ...
                            st.error(
                                "Ты нарушил обещание, чёрт тайлер что здесь происходит почему меня принимают за тебя?!Сядь. Так почему меня принимают за тебя? А ты не знаешь? нет не знаю, нет знаешь, нет,нет ... Я не знаю, вспомни, почему все вокруг путают нас? Потому что, потому что..Потому что мы одни человек? Правильно.")
                else:
                ...
                if rem <= 0:
                    ...
                    if st.session_state.my_name == data["explainer"]:
                        st.success("ТИ ПОЯСНЮЄШ!")
                        st.markdown(f'<div class="word-box">{data["word"].upper()}</div>', unsafe_allow_html=True)
                        c1, c2 = st.columns(2)
                        if c1.button("✅ ВГАДАНО"):
                            data["scores"][st.session_state.my_name] = data["scores"].get(st.session_state.my_name,
                                                                                          0) + 1
                        ...
                        if c2.button("❌ СКІП"):
                            ...
                            st.rerun()
                    elif st.session_state.my_name == data["listener"]:
